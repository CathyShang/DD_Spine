...
def fit(module: torch.nn.Module, train_data, valid_data, optimizer, max_step, loss, metrics: list, is_higher_better,
    ...
    if checkpoint_dir is not None:
        checkpoint_dir = os.path.join(checkpoint_dir, time.strftime('%y%m%d%H%M%S'))        # str(time.time())
    ...
    try:
        while step < max_step:
            ...
            with torch.no_grad():
                epoch = int(step / step_per_batch)
                print('epoch {} step {} train {}: {}'.format(epoch,
