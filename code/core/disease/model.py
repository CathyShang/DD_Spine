...
class DiseaseModelBase(torch.nn.Module):
    ...
    def _gen_annotation(study: Study, vertebra_coords, vertebra_scores, disc_coords, disc_scores) -> dict:
        """
        Used to generate the annotation for submitting.
        :param study:
    ...
    def _train(self, sagittals, _, distmaps, v_labels, d_labels, v_masks, d_masks, t_masks) -> tuple:
        ...
        kp_loss = self.backbone(sagittals, distmaps, masks)
        #
        # # disease classification training, if class_model from t_class_model, the input should be transverse
        # class_loss = self.class_model(sagittals, v_labels, d_labels, t_masks)
    ...
    def _inference(self, study: Study, to_dict=False):
        ...
        # d_score = self.class_model(sagittal, v_coord, d_coord)
