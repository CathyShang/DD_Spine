...
class Study(dict):
    def __init__(self, study_dir, pool=None):
        ...
        if self.t2_sagittal_uid is None:    # 找T2矢状面所在的Series
            for series_uid, series in self.items():
        ...
        if self.t2_transverse_uid is None:  # 找T2横断面所在的Series
            for series_uid, series in self.items():
    ...
    def t2_transverse_k_nearest(self, pixel_coord, k, size, max_dist, prob_rotate=0,
        ...
        :param k: 对于每个标注点，取k个距离该点最近的横断面图像，
        :param size:
        :param max_dist: k个点中只取距离小于max_dist的
        :param prob_rotate: 数据增强中图像裁切所用参数
        :param max_angel: 数据增强中图像裁切所用参数
        ...
        if k <= 0 or self.t2_transverse is None:
            ...
            return images, masks
        human_coord = self.t2_sagittal_middle_frame.pixel_coord2human_coord(pixel_coord)       # 转化到实际物理坐标系
        dicoms = self.t2_transverse.k_nearest(human_coord, k, max_dist) # 得到距离human_coord中每个标注点最近的横断面图像，每个点取k个距离最近的图像
        images = []
        masks = []
        for point, series in zip(human_coord, dicoms):
            ...
            for dicom in series:
                if dicom is None:
                    ...
                    projection = dicom.projection(point)        # 投影到横断面dicom图像上的坐标
                    image, projection = dicom.transform(
                        projection, size=[size[0]*2, size[1]*2], prob_rotate=prob_rotate, max_angel=max_angel,  # 先将图像放大
                        tensor=False
                    )
                    image = tf.crop(
                        image, int(projection[0]-size[0]//2), int(projection[1]-size[1]//2), size[0], size[1])  # 后裁剪，img, top, left, height, width
                    image = tf.to_tensor(image)
                temp_images.append(image)
            temp_images = torch.stack(temp_images, dim=0)       # 存储每个点对应的多张横断面图像
            images.append(temp_images)
            masks.append(temp_masks)
        images = torch.stack(images, dim=0)                     # 存储所有点对应的所有横断面图像
